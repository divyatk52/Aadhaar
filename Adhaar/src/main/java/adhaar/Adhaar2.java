...
public class Adhaar2 {
	...
	public static class MapperClassSort extends Mapper<Object, Text, Text, Text> 
	{
 		...
 		public void map(Object key, Text record, Context con) throws IOException, InterruptedException 
		{
   			con.write(new Text("1"), record); // we are using the same key so that all the values reaches the same reducer
 		}
	}
	...
	public static void main(String[] args) throws Exception 
	{
		...
		Job job;
    		...
    		job.setJarByClass(Adhaar2.class);
		...
		job.setOutputValueClass(Text.class);
    		...
    		FileOutputFormat.setOutputPath(job, new Path("/user/cloudera/opTemp25/"));
		...
		job.setOutputValueClass(Text.class);
    		...
    		FileInputFormat.addInputPath(job, new Path("/user/cloudera/opTemp25/"));
    		FileOutputFormat.setOutputPath(job, new Path("/user/cloudera/Finalout2/"));
